...
BOOL CMFCTESTDlg::SelectDB()
{
	...
	CString query;
	//条件全部为空则查询所有书籍
	if (m_queryBook.IsEmpty() && m_queryAuthor.IsEmpty() && 
	...
	else if (!m_priceMin && !m_priceMax)//价格范围为空
	{
		...
		query.Format(TEXT("select * from book where price >= %f and price <=%f;"),
			m_priceMin,m_priceMax);
	//查询数据
...
BOOL CMFCTESTDlg::InsertDB()
{
	...
	CString query;
	query.Format(TEXT("insert into book values('%s','%s','%s','%s',%.2f);"),
				m_addBook, m_addAuthor, m_addPublisher, m_addTime, m_addPrice);
...
void CMFCTESTDlg::GetDataFromDB()
{
	...
	//检索一个结果集合的下一行。当在mysql_store_result()之后使用时，
